import { nationMatches } from "../../domain/evaluation/benchmarkRanges.js";
import type {
  ManualCuratedRating,
...
export function sevenAZeroManualReferencesAsFloors(): ManualCuratedRating[] {
  return SEVEN_A_ZERO_MANUAL_REFERENCES.map((reference): ManualCuratedRating => {
    const aliases = "aliases" in reference ? reference.aliases : undefined;
    return {
      id: `manual-floor-${reference.id}`,
      nameSearch: reference.playerName,
      ...(aliases ? { aliases } : {}),
      nationCode: reference.nationCode,
      worldCupYear: reference.worldCupYear,
      floor: Math.max(55, reference.referenceOverall - reference.tolerance),
      reason: `Manual 7a0 reference floor: ${reference.referenceOverall} +/- ${reference.tolerance}.`
    };
  });
}
