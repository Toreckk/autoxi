...
export function detectAnomalyDetails(
  ...
  for (const card of cards) {
    if (card.primarySource === "FJELSTUL_GENERATED" && card.overall >= 90 && !hasStrongHighRatingSignal(card)) {
...
function hasStrongHighRatingSignal(card: RatingLabCardReport): boolean {
  return card.awards.length > 0 || card.goals >= 4 || card.appearances >= 6 || card.reasons.includes("key contributor");
}
